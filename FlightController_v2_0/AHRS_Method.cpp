...
vector3Float* MadgwickAHRS::update()
{
	...
	// in radians
	angles.y = atan2f(q0 * q1 + q2 * q3, 0.5f - q1 * q1 - q2 * q2);		// roll
	angles.x = asinf(-2.0f * (q1 * q3 - q0 * q2));						// pitch
	angles.z = atan2f(q1 * q2 + q0 * q3, 0.5f - q2 * q2 - q3 * q3);		// roll (heading)
...
vector3Float* MadgwickIMU::update()
{
	// Convert gyroscope degrees/sec to radians/sec
	gx *= 0.0174533f;
	gy *= 0.0174533f;
